...
# -------------------
# Initialize SentenceTransformer ONCE with device CPU
# -------------------
device = "cpu"
embedder = SentenceTransformer("paraphrase-MiniLM-L3-v2")  # no device here


# -------------------
# Initialize HuggingFace QA pipeline for free, local inference
# -------------------
...
dimension = doc_embeddings.shape[1] if doc_embeddings.ndim == 2 else 0
if dimension == 0:
    raise ValueError('Cannot create FAISS index with dimension 0.')
...
def ai_response(message):
    ...
    if dimension == 0:
        return "Sorry, no service information available right now. Please try again later.", []
    
    if query_vec.shape[1] != dimension:
        return "Sorry, technical issue processing your query. Please try rephrasing.", []
